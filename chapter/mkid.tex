...
$N$個のMKIDの多重読み出しを考える。個々のMKIDの共振周波数$f_{i}$（$=2\pi\omega_{i}$）とし、LOの周波数を$f_{j}$（$=2\pi\omega_j$）とする。

DACから出力した多重化した正弦波$\sum_{i=0}^{N}\sin \omega_{i}$と余弦波$\sum_{i=0}^{N}\cos \omega_{i}$は、ミキサーによってLOの信号と混合する。混合した信号は合成され、このフィード信号$S_{\mathrm{in}}$がMKIDに入力する。
\begin{equation}
 \begin{split}
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=15cm]{figures/ddc.pdf}
 \caption{}
 \label{fig:ddc}
\end{figure}
