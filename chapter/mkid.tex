...
\end{aligned}
\end{equation}
ここで、$\mathcal{A}_{j}:=A{''}_{j}/2$とし、$\omega_{\mathrm c}{'}t \gg \phi_{i}$となるカットオフ周波数$\omega_{\mathrm c}'$を選んだ。%このLPFは実際的には、二つの要素がある。ひとつは、FPGAに実装するCIC（Cascaded Integration-Comb）フィルター、もうひとつは、時間積分（観測時間を増やす・平均化する）によるものである。
