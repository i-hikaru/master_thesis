...
アップ・コンバートとダウン・コンバートを含むフィード信号の合成・分離と、MKIDの共振による信号の状態変化を図\ref{fig:iq_md}に模式する。図\ref{fig:readout_sys}のI/Qミキサー（I/Q Mix）は、構成要素に分解すると、2つの混合器とハイブリッド・カプラー、ローパス・フィルターからなる。
...
\begin{equation}
 \mathrm{Hyb}^{-1}\left(S_{\mathrm{feed}}'(t)\right)
  = \begin{cases}     
    ...
    \end{cases}
\end{equation}
分解した信号$S_{1}'(t)$、$S_{2}'(t)$は、再度MixでLO信号$S_{3}(t)$と混合する。
\begin{equation}
 \begin{aligned}
  S_{13}'(t) &:= \mathrm{Mix}(S_{1}'(t), S_{3}(t))
...
%   + \cos\left[(\omega_{i} + 2\omega_{\mathrm{LO}})t + \phi_{i}\right]
%   \biggr\} \\
  I(t) &:= \mathrm{LPF}(S_{13}'(t), \omega_{\mathrm c})
  = \sum_{i=1}^{N}\frac{A_{i}''}{2} \cos(\omega_{i}t + \phi_{i}) \\
%   S_{23}'(t) &:= \mathrm{Mix}(S_{2}'(t), S_{3}(t))
%   = \sum_{i=1}^{N}\frac{A_{i}''}{2} \biggl\{\sin(\omega_{i}t + \phi_{i})
%   + \sin\left[(\omega_{i} + 2\omega_{\mathrm{LO}})t + \phi_{i}\right]
%   \biggr\} \\
  Q(t) &:= \mathrm{LPF}(S_{23}'(t), \omega_{\mathrm c})
  = \sum_{i=1}^{N}\frac{A_{i}''}{2} \sin(\omega_{i}t + \phi_{i})
  \label{eq:adc_input}
 \end{aligned}
\end{equation}
ここで、$A_{i}'':=A_{i}'A$とし、$\omega_{\mathrm c}$（$\sim 2\pi \times 500$ MHz）は、$\omega_{\mathrm{LO}}$（$\sim 2\pi \times 4$ GHz）成分を含む高調波を落とすカットオフ周波数である。

\subsubsection{復調}

\begin{figure}[!h]
 \centering
 \includegraphics[width=15cm]{figures/demodulation.pdf}
%  \caption{ADCへ入力した信号は、LO信号による高調波成分を含んでいる。MKIDの信号を取り出すために、デジタル基板上のFPGAにLPF（ローパス・フィルター）を実装する。}
 \caption{デジタル基板上（FPGA）でのMKID信号の復調}
 \label{fig:lpf_fpga}
\end{figure}

\clearpage

図\ref{fig:lpf_fpga}に示すように、ADCに入力した信号は、FPGAに実装したDDS（Direct Digital Synthesizers）で生成される信号
$S_{I}(t):=\cos \omega_{j}t,\ S_{Q}(t):=\sin \omega_{j}t$（$j=1,\ 2,\ \cdots{,}\ N$）
% \begin{equation}
%  \begin{aligned}
...
%  \notag \\ &\
 ...
 \label{eq:QQ}
\end{align}
式(\ref{eq:II})と(\ref{eq:QQ})、式(\ref{eq:IQ})と(\ref{eq:QI})の信号をそれぞれ合成すると、
\begin{equation}
 \begin{aligned}
  I'(t) &:= 
  \sum_{i=1}^{N}
 ...
 \end{aligned}
\end{equation}
となる。ここで、$Q'(t)$信号は、$I_{Q}(t)$信号の位相を$\pi$回転して（つまり、(-1)倍して）から$Q_{I}(t)$と合成している。
これより、$I'(t)$、$Q'(t)$信号の第二項以下の高調波をLPFで落とせば、$j$番目のMKIDにより変調された振幅$A_{j}''$と位相$\phi_{j}$の情報が同時に読み出せる。
\begin{equation}
\begin{aligned}
 \mathcal{I}_{j}(t) &:= \mathrm{LPF}(I'(t), \omega_{\mathrm c}')
  = \mathcal{A}_{j} \cos \phi_{j} \\
 \mathcal{Q}_{j}(t) &:= \mathrm{LPF}(Q'(t), \omega_{\mathrm c}')
  = \mathcal{A}_{j} \sin \phi_{j} 
 \label{eq:mkid_sig}
\end{aligned}
\end{equation}
ここで、$\mathcal{A}_{j}:=A{''}_{j}/2$とし、$\omega_{\mathrm c}{'}t \gg \phi_{i}$となるカットオフ周波数$\omega_{\mathrm c}'$を選んだ。
...
複素平面上の実軸と虚軸にとると、図\ref{fig:mkid_real}(a)のような円が描ける。したがって、振幅$\mathcal{A}_{j}$と位相$\phi_{j}$は、次のように表せる。
\begin{equation}
\begin{aligned}
 \mathcal{A}_{j}
...
\begin{figure}[!h]
 ...
 \caption[既存の読み出し系によるMKIDの読み出し例]{(a) 実際に読み出したMKIDの信号を複素平面上に描写した図。振幅読み出し(b)と位相読み出し(c)。これらは後述(2.4)のフロントエンド回路を使って測定した。}
 \label{fig:mkid_real}
\end{figure}
