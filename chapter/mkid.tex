...
\begin{equation}
 \begin{aligned}
  ...
  \biggr\} \\
%   &\sim \sum_{i=1}^{N}\frac{A_{i}''}{2} \cos(\omega_{i}t + \phi_{i})
  S_{23}'(t) := \mathrm{Mix}(S_{2}'(t), S_{3}(t))
...
% ADCに入力した信号はFPGAに実装したDDS（Direct Digital Synthesizers）の信号$S_{\mathrm{I}}(t):=\sum_{j=1}^{N} \cos \omega_{j}t$、$S_{\mathrm{Q}}(t):=\sum_{j=1}^{N} \sin \omega_{j}t$をそれぞれMixで混合する。
% \begin{equation}
%  \begin{aligned}
%   S_{13}{''}(t) := \mathrm{Mix}(S_{13}{'}(t), S_{\mathrm{I}})
...
%   + \cos[(\omega_{i} - \omega_{j} + 2\omega_{\mathrm{LO}})t + \phi_{i}]
%   \biggr\} \\
%   S_{23}{''}(t) := \mathrm{Mix}(S_{13}'(t), S_{\mathrm{Q}})
%   &= \sum_{i, j=1}^{N} \frac{A_{i}{''}}{4}
%   \biggl\{
%     \cos[(\omega_{i} + \omega_{j})t + \phi_{i}]
%   + \cos[(\omega_{i} - \omega_{j})t + \phi_{i}] \\
...
%  \includegraphics[width=16cm]{figures/lpf_fpga.pdf}
 \includegraphics[width=15cm]{figures/down_sampling.pdf}
...
\begin{figure}[!h]
 ...
 \caption[既存の読み出し系によるMKIDの読み出し例]{(a) 実際に読み出したMKIDの$I$、$Q$信号を複素平面上に描写した図。振幅読み出し(b)と位相読み出し(c)。これらは後述(2.4)のフロントエンド回路を使って測定した。}
 \label{fig:mkid_real}
\end{figure}
