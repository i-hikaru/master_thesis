...
1) の最も簡単な解決案は、問題となるLPFを実装しないことである。これにより、原理的にはDACの最大出力周波数（サンプリング周波数の半分）まで、帯域を広げることができる。LPFを外すことにより、高周波ノイズの漏れ込みが懸念されるが、これはDACの出力後、コネクターの先にユーザー定義のLPF（付録\ref{app:lpf}）を取り付けることで、帯域を制限することなく容易に対応できる。
