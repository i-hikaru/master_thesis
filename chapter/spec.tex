...
\subsection{帯域に対する要求}\label{subsec:req_band}

1) の最も簡単な解決案は、問題となるLPFを実装しないことである。これにより、原理的にはDACの最大出力周波数（サンプリング周波数の半分）まで、帯域を広げることができる。LPFを外すことにより、高周波ノイズの漏れ込みが懸念されるが、これはDACの出力後、
% コネクターの先にユーザー定義のLPF（付録\ref{app:lpf}）を取り付けることで、
コネクターの先に高いカットオフ周波数をもつユーザー定義のLPF（付録\ref{app:lpf}）を取り付けることで、
帯域を制限することなく容易に対応できる。
...
\subsection{ユーザビリティに対する要求}\label{subsec:userbirity}

3) はユーザビリティを損なう原因である水晶発振器を、ユーザーが直感的に使うことができる周波数分解能にするために整数倍の周波数をもつ水晶発振器に変更する（\ref{subsubsec:multiplex}を参照）。これにより、ユーザーはある数（MKIDの共振周波数）に12.288を掛けたり、割ったりするような認知的負荷から開放される。ADC/DACの入出力先のコネクターの規格もSMAに統一することで作業効率に改善をもたらす。
...
要求分析に基いて、新しいアナログ基板RHEA\footnote{RHEAは、「Rhea is a High spEed Analog board」の略で、南米に生息する陸鳥（ground bird）から名前を付けた。}の仕様を策定する。読み出し系の性能を決める上で、広帯域性（高速化・高周波化）と省電力性（低電圧化・低電流化）をどう両立させるかが重要な点となる。
