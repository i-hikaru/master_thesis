...
の等張配線は$\pm 1$ mmの精度である。また、対ではないが同種の信号線
%（たとえば、同回路図のDACのデータ線「{\tt DAC\_\,P\_\,D1}」と「{\tt DAC\_\,P\_\,D2}」）は
は$\pm 2$ mmの精度である。したがって、実際の基板における等張配線の精度は十分満足する。

\begin{figure}[!h]
 \centering
 \includegraphics[width=13cm]{figures/eq_wire.pdf}
 \caption{FMC LPCからDACへのデータ信号の等張配線。3対のデータ線が等張配線されていることが確認できる。それぞれの対は$\pm$1 mmの精度で、対同士（ここでは3対）は$\pm$2 mmの精度で等張配線される。残りの4対は内層で配線される。}
 \label{fig:eq_wire}
\end{figure}
